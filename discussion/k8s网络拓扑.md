...
当前是**混合拓扑**：7 个无状态业务服务在 kind 里；Postgres / Kafka / Redis / Casdoor / APISIX 仍在 Docker Compose。
...
flowchart TB
  ...
  subgraph kind["kind 节点（一个 Docker 容器）"]
    subgraph ns["namespace: vpp"]
      ...
      svcOPT["Service optimization<br/>ClusterIP"]
    end
...
端口号没变：resource 仍是 8082/5002，gateway 8083/5005，telemetry 5003，dispatch 5006，simulator 8084，alarm 8087，optimization 8088。变的是前面的 IP。

simulator 绑 `0.0.0.0:8084`。alarm 人管面 HTTP `:8087`，仅 ClusterIP。optimization HTTP `:8088`（仅 `/healthz`），仅 ClusterIP。
...
| optimization | telemetry gRPC | `telemetry.vpp.svc.cluster.local:5003`     |
| optimization | resource gRPC  | `resource.vpp.svc.cluster.local:5002`      |
| optimization | dispatch gRPC  | `dispatch.vpp.svc.cluster.local:5006`      |
...
simulator 与 alarm、optimization 故意不映射：simulator 只被集群内 gateway 调用；alarm 人管面 v1 用 `kubectl -n vpp port-forward svc/alarm 8087:8087`，不挂 APISIX；optimization 无入站业务 API，联调用 `kubectl -n vpp port-forward svc/optimization 8088:8088`。resource / gateway 的内部 gRPC 也各有一个未映射的 NodePort（30502 / 30505），只是为了避免 kube-proxy 抢走上面四个口；集群内互调仍走 ClusterIP。
...
| optimization | 仍是 ClusterIP，host `:8088` 不通；联调 port-forward |
