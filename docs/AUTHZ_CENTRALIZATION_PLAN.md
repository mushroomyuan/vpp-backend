...
| 缩写 | 全称 | 职责 | 当前在哪 | 改造后在哪 |
|---|---|---|---|---|
| **PAP** | Policy Administration Point | 维护"角色 → 权限"的绑定关系（谁能做什么） | 散落在各服务代码里 | **Casdoor**（Permissions 页面 / API） |
| **PDP** | Policy Decision Point | 给定 `(subject, object, action)` 返回 allow/deny | 各服务 `allowRBAC` 硬编码判断 | **各服务本地 Casbin Enforcer**（策略数据源自 Casdoor） |
| **PEP** | Policy Enforcement Point | 拦截请求、组装判断入参、执行 PDP 的结果 | `AuthMiddleware` | **保留**，只是判断逻辑改为调用 PDP，不再自己写业务规则 |
...
| `authz_decision_total{result="allow|deny|degraded_allow|degraded_deny"}` | 观察降级模式实际影响范围 |
...
服务启动时（或 CI/CD，**C9**）以 upsert 方式调用 Casdoor `add-permission` / `update-permission`，把目录同步过去；**绑定哪个角色能用哪个权限**完全在 Casdoor 侧管理，不需要改服务代码。C5 阶段目录与角色绑定通过 [`deploy/casdoor/conf/init_data.json`](../deploy/casdoor/conf/init_data.json) 种子手工落库（见 §九 C5）。
...
| **C8** | 观测性落地：`authz_policy_sync_*` 指标 + 告警规则；明确写进 runbook（同步失败时运维该做什么） | C7 | 未开始 |
| **C9** | 权限目录自动化注册（服务启动/CI upsert 到 Casdoor），替代手工维护 | C7 | 未开始 |
...
- [ ] `authz_policy_sync_last_success_timestamp` 等指标可在 `/metrics` 观察到 — **C8**
