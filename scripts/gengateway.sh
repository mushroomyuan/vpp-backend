...
ENTRY_PROTO="${PROTO_ROOT}/resource_service.proto"
...
generate() {
  ...
  log "generating grpc-gateway reverse proxy stubs from ${ENTRY_PROTO}"
  protoc \
    -I="${PROTO_ROOT}" \
...
# When buf is available, resource grpc-gateway is already generated by buf.gen.yaml.
if command -v buf >/dev/null 2>&1; then
  log "buf detected; running buf generate for resource (includes grpc-gateway)"
  (
    cd "${PROTO_ROOT}"
    if [[ ! -f buf.lock ]]; then
      buf dep update
