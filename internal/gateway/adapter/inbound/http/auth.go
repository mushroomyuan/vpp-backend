...
// AuthConfig controls Gateway HTTP identity / authorization middleware (C10b).
type AuthConfig struct {
	// TrustProxyHeaders when true requires a valid X-Userinfo header.
	// When false, auth is fully bypassed (local direct :8083 debug).
...
// when TrustProxyHeaders is true. Mount only on mappings routes — not on EMS ingest.
func AuthMiddleware(
	cfg AuthConfig,
	...
	return func(c *gin.Context) {
		...
		if err != nil {
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{
				"error": "missing or invalid X-Userinfo (enable APISIX OIDC on mappings or set auth.trust-proxy-headers: false for local debug)",
			})
			return
		}
