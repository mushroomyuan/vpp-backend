...
func (a *App) Run() {
	if err := a.cmd.Execute(); err != nil {
		logrus.Fatalf("vpp-optimization: %v", err)
	}
}
...
func NewApp(basename string) *App {
	...
	cmd := &cobra.Command{
		Use:   basename,
		Short: "VPP Optimization Service",
		Long: `VPP Optimization Service is the internal decision loop: it reads
Telemetry/Resource over gRPC, evaluates threshold rules, and submits
automatic tasks to Dispatch. v1 has no inbound business API.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func runApp(opts *options.Options) error {
	...
	return Run(config.CreateFromOptions(opts))
}
...
func loadViperConfig() {
	if cfgFile := viper.GetString("config-file"); cfgFile != "" {
		...
		viper.SetConfigName("optimization")
